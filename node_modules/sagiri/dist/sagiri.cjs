...
const nodeFetch__namespace = /*#__PURE__*/_interopNamespaceCompat(nodeFetch);
...
if (globalThis.fetch === void 0) {
  fetchFn = nodeFetch__namespace.default;
} else {
...
const sagiri = (token, defaultOpts = { results: 5 }) => {
  ...
  return async (file, opts = {}) => {
    ...
    const form = new FormData__default();
    ...
    if (typeof file === "string") {
      if (/^https?:\/\//.test(file)) {
        ...
        form.append("file", node_fs.createReadStream(file), { filename: "image.jpg" });
      }
    } else if (file instanceof node_buffer.Buffer) {
      form.append("file", file, { filename: "image.jpg" });
    } else if (file instanceof node_stream.Readable) {
      form.append("file", file, { filename: "image.jpg" });
