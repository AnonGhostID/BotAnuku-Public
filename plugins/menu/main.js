...
let handler = async (m, { conn, usedPrefix: _p, __dirname, command, isPrems }) => {
	try {
		...
		let nais = await (await fetch('https://raw.githubusercontent.com/AnonGhostID/menupic/main/img.json')).json().then(v => v.getRandom())
		...
		/*if (!/all/.test(command) && await getDevice(m.key.id) == 'android') {
			...
			//await conn.sendList(m.chat, 'Hello '+name, txtList, pauthor, 'LIST MENU', nais, sections, m, [['neko', '.neko', 'quick_reply'], ['copy ini', 'https://cerdas.com', 'cta_copy'], ['owner', 'https://wa.me/6281238030439', 'cta_url']])
