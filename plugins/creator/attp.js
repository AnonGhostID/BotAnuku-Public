...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
	...
	try {
		...
		let stiker = await createSticker(`https://api.lolhuman.xyz/api/${command}?apikey=${apiKey}&text=${encodeURIComponent(text.substring(0, 151))}`, { pack: packname, author: author })
